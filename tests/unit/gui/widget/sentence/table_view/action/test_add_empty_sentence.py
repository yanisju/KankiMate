...
from src.gui.widget.sentence.table_view.action.add_empty_sentence import AddEmptySentenceAction
...
def action(sentence_table_view_menu, vocabulary_manager):
    return AddEmptySentenceAction(
        sentence_table_view_menu,
        vocabulary_manager,
    )

def test_add_empty_sentence_action(qtbot, action: AddEmptySentenceAction, vocabulary_manager, sentence_table_view):
    
    sentence_manager = action.parent().parent().model().sentence_manager
    
    initial_count = len(sentence_manager)
    action.trigger()
    assert len(sentence_manager) == initial_count + 1
